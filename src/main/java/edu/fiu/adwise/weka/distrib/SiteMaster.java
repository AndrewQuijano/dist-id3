...
public class SiteMaster extends java.rmi.server.UnicastRemoteObject implements RmtMaster {
    boolean debug = true;
    /**
     * The number of sites
     */
    ...
    private static final String firstToken = "S1L1";
    ...
    private Instance getInstance(Instance inst) {
        ...
        return g_instHashtbl.get(inst.stringValue(att));
    }

    public String updateToken(int siteNum, String token) throws RemoteException {
        m_globalSiteCounter++;
        return "S" + siteNum + "L" + m_globalSiteCounter;
    }
     ...
     */
    private double computeInfoGain(double origInfo, int numOrigInsts,
                                   Attribute att) throws RemoteException {
...
// For now just setting all classes to be equiprobable.. This is clearly wrong
    ...
    public void prepareTransVector() throws RemoteException {
        for (int i = 0; i < m_insts.numInstances(); i++) {
            if (requirementsMet(m_insts.instance(i))) {
    ...
    private void prepareTransVectorWithClassFilter(double classval) {
        for (int i = 0; i < m_insts.numInstances(); i++) {
            ...
            Array.setBoolean(m_transVector, i, inst.classValue() == classval && requirementsMet(inst));
        }
    }
    ...
    static public void main(String[] args) throws Exception {
        ...
        SiteMaster mySelf = new SiteMaster(Integer.parseInt(siteNum));
