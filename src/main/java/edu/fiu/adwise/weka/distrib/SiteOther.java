...
public class SiteOther extends java.rmi.server.UnicastRemoteObject implements RmtSlave {
     ...
     */
    static Hashtable<String, Instance> g_instHashtbl;
     ...
     */
    protected Hashtable<String, TreeNode> m_classificationTbl;

    /**
     * Local stack which helps simulate the global stack
     */
    Stack<InfoObject> m_requirementStack;

    /**
     * Local stack which helps simulate the global stack for attributes
     */
    Stack<Attribute> m_attributeStack;

    /**
     * The classifying attribute stack
     */
    private final Stack<classInfo> m_currClassAttrInfoStack;
     ...
     */
    private final Object m_transVector;

    /**
     * The set used for intersection purposes
     */
    private Set<BigInteger> m_set;

    /**
     * The final encrypted sets used for intersection
     */
    private Set<BigInteger>[] m_enc_sets;
    ...
    SiteOther(int siteNum) throws Exception {
        ...
        m_hmorphkeys = new Dense(SiteOther.txt_blk_size, SiteOther.cip_blk_size, SiteOther.prec, SiteOther.show);
    ...
    public void linkMe(int nhosts, String[] hosts, KeyedInstances data, BigInteger
        ...
        try {
            m_Master = (RmtMaster) Naming.lookup("rmi://" + hosts[0] + "/Site1");
            for (int i = 1; i < m_nhosts; i++) {
                if ((i + 1) == m_siteNum) {
                    m_Slaves[i - 1] = this;
                } else {
                    String lookupSite = "rmi://" + hosts[i] + "/Site" + (i + 1);
    ...
    private boolean isNumAttributesZero(String token) throws
        ...
        if (m_siteNum == 2) {
            ...
            comp1 = m_Slaves[m_siteNum - 3].encryptMe(m_phellman.Encrypt(disguisedNumAttr));
            comp2 = m_phellman.Encrypt(m_Slaves[m_siteNum - 3].returnEncDisguisedNumAttr());
        }
    ...
    public void prepareTransVector() throws RemoteException {
        for (int i = 0; i < m_insts.numInstances(); i++) {
            Array.setBoolean(m_transVector, i, requirementsMet(m_insts.instance(i)));
        }
    }
    ...
    private boolean requirementsMet(Instance inst) {
        for (int i = 0; i < m_requirementStack.size(); i++) {
            ...
            if (!instValue.equals(io.returnValue()))
                return false;
        }
    ...
    public double locallyClassifyInstance(Instance inst, String token) throws
            RemoteException {
        TreeNode node = m_classificationTbl.get(token);
        if (node instanceof LeafNode) {
            ...
            int siteNum =
                    Integer.parseInt(siteToken.substring(startindex, endindex));
    ...
    public double[] localDistributionForInstance(Instance inst, String token) throws RemoteException {
        ...
        if (node instanceof LeafNode) {
            ...
            int startindex = siteToken.lastIndexOf("S") + 1;
            int endindex = siteToken.lastIndexOf("L");
            int siteNum = Integer.parseInt(siteToken.substring(startindex, endindex));
    ...
    public double attribMaxInfoGain() throws RemoteException {
        ...
        while (attEnum.hasMoreElements()) {
            ...
            if (!att.equals(m_insts.classAttribute()) &&
                    !att.name().equals("_keyAttr") &&
                    !m_attributeStack.contains(att)) {
...
// at this point who reinitialized master?
    ...
    public String buildClassifier(String token) throws RemoteException {
        ...
        try {
            if (isNumAttributesZero(token)) {
                ...
                return outToken;
            } else if (allTransHaveSameClass(token) /*all have same class*/) {
                if (debug)
                ...
                if (Utils.eq(infoGains[maxIndex], 0)) {
                    ...
                    System.out.println("Creating leaf at " + token + " ClassValue: " + ((LeafNode) leaf).getClassValue());
                    m_classificationTbl.put(outToken, leaf);
                    return outToken;
                } else {
                    if (maxIndex == 0) {
    ...
    public void locallyBuildClassifier(String token) throws RemoteException {
        ...
        for (int i = 0; i < m_classifyingAttribute.numValues(); i++) {
            ...
            classInfo c = m_currClassAttrInfoStack.pop();
    ...
    public void prepareSet() throws RemoteException {
        ...
        for (int i = 0; i < Array.getLength(m_transVector); i++) {
            if (Array.getBoolean(m_transVector, i)) {
    ...
    public int SetIntersect() throws RemoteException {
        ...
        Set<BigInteger> int_set = new HashSet<>(m_enc_sets[0]);
        for (int i = 1; i < m_nhosts; i++) {
    ...
    public void EncryptAndSendOn(Set<BigInteger> in_set, int nhosts, int destSite) throws RemoteException {
        Set<BigInteger> out_set = new HashSet<>(in_set.size());
        for (BigInteger o : in_set) {
            out_set.add(m_phellman.Encrypt(o));
        }
    ...
    public void returningSet(Set<BigInteger> fin_set, int sourceSiteNum) throws
            RemoteException {
        m_enc_sets[sourceSiteNum - 1] = fin_set;
    }

    static private void registerTestInstances() {
        g_instHashtbl = new Hashtable<String, Instance>();
    ...
    static public void main(String[] args) throws Exception {
        ...
        BufferedReader instancesReader = new BufferedReader(new FileReader(instancesFileName));
        ...
        SiteOther mySelf = new SiteOther(Integer.parseInt(siteNum));

        try {
            System.out.println("START");
            System.out.println("ABOUT TO BIND");
            Naming.rebind("Site" + siteNum, mySelf);
